...
app.controller('mainCtrl', function($scope, itunesService){
  ...
  //input box on index.html, head over there and check if that input box is tied to any specific model we could use.
  ...
  $scope.getSongData = function() {
    itunesService.getData($scope.artist).then(function(dataFromService) {
      ...
      $scope.songData = myFinalArray.slice(0, myFinalArray.length);
    });
  };


  //Check that the above method is working by entering a name into the input field on your web 
  //app, and then console.log the result
  ...
  //is sort the data you got back to be an object in the following format.
    /*
      ...
      Type: "song"
  */
  //the iTunes API is going to give you a lot more details than ng-grid wants. Create a new 
  //array and then loop through the iTunes data pushing into your new array objects that look like the above data.
